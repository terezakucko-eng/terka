...
const MARKETING_CHANNELS_HTML = `<!DOCTYPE html>
<html lang="cs">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>Marketingové kanály — Růžový slon</title>
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  body { font-family: 'Segoe UI', sans-serif; background: #fdf6f9; color: #2d1f28; min-height: 100vh; }
  ...
  .controls span { font-size: 0.8rem; color: #888; margin-right: 4px; }
  .filter-btn {
    padding: 5px 14px;
    border-radius: 20px;
  ...
  .filter-btn:not(.active) { background: #f5f5f5; color: #555; }
  ...
  .matrix {
    ...
    height: 100%;
    position: relative;
    border-left: 2px solid #e0c4d0;
    border-bottom: 2px solid #e0c4d0;
  ...
  .axis-x-lo { bottom: -28px; left: 4px; }
  .axis-x-hi { bottom: -28px; right: 4px; }
  .axis-y-lo { left: -60px; bottom: 4px; transform: rotate(-90deg); transform-origin: right center; }
  .axis-y-hi { left: -60px; top: 4px; transform: rotate(-90deg); transform-origin: right center; }
  .quadrant-label {
    position: absolute;
    font-size: 0.68rem;
    color: #ddb8cc;
  ...
  .bubble.highlighted { opacity: 1; }

  .sidebar {
    width: 300px;
    background: white;
    border-left: 1px solid #f0dde8;
    overflow-y: auto;
    padding: 20px;
  }
  .sidebar h2 { font-size: 0.9rem; color: #c2185b; margin-bottom: 16px; font-weight: 700; }
  ...
  .channel-card.selected { border-color: currentColor; }

  .card-header { display: flex; align-items: center; gap: 8px; margin-bottom: 4px; }
  .card-icon { font-size: 1.1rem; }
  .card-name { font-size: 0.82rem; font-weight: 700; }
  .card-cat { font-size: 0.65rem; text-transform: uppercase; letter-spacing: 0.06em; opacity: 0.7; }
  .card-desc { font-size: 0.75rem; color: #666; line-height: 1.4; margin-top: 4px; }
  ...
  .tooltip h3 { font-size: 0.88rem; font-weight: 700; margin-bottom: 4px; }
  .tooltip p { font-size: 0.75rem; color: #666; line-height: 1.4; }
...
<header>
  ...
  <div>
    <h1>Marketingové kanály — Růžový slon</h1>
    <p>Interaktivní mapa podle dopadu a námahy · 21 kanálů</p>
  </div>
</header>

<div class="controls">
  <span>Filtr:</span>
  <button class="filter-btn active" data-cat="all" style="border-color:#c2185b;background:#c2185b;color:white">Vše</button>
  <button class="filter-btn" data-cat="owned" style="--c:#7b1fa2">Vlastní kanály</button>
  <button class="filter-btn" data-cat="search" style="--c:#1565c0">Vyhledávání</button>
  <button class="filter-btn" data-cat="paid" style="--c:#e65100">Placená reklama</button>
  <button class="filter-btn" data-cat="partner" style="--c:#2e7d32">Partnerské</button>
  <button class="filter-btn" data-cat="offline" style="--c:#4a148c">Offline & Zážitek</button>
  <button class="filter-btn" data-cat="community" style="--c:#00838f">Komunita & Obsah</button>
</div>

<div class="legend">
  <div class="legend-item"><div class="legend-dot" style="background:#7b1fa2"></div>Vlastní kanály</div>
  <div class="legend-item"><div class="legend-dot" style="background:#1565c0"></div>Vyhledávání</div>
  <div class="legend-item"><div class="legend-dot" style="background:#e65100"></div>Placená reklama</div>
  <div class="legend-item"><div class="legend-dot" style="background:#2e7d32"></div>Partnerské</div>
  <div class="legend-item"><div class="legend-dot" style="background:#4a148c"></div>Offline & Zážitek</div>
  <div class="legend-item"><div class="legend-dot" style="background:#00838f"></div>Komunita & Obsah</div>
  <div class="legend-item" style="margin-left:8px"><div class="legend-dot" style="background:#ccc;border:1px solid #aaa"></div>Velikost = potenciál</div>
</div>

<div class="main">
  <div class="matrix-wrap">
    <div class="matrix" id="matrix">
      <div class="grid-line" style="width:100%;height:1px;top:50%;left:0;opacity:0.5"></div>
      <div class="grid-line" style="width:1px;height:100%;top:0;left:50%;opacity:0.5"></div>
      <div class="quadrant-label" style="top:8px;left:8px">Vysoký dopad / Nízká námaha</div>
      <div class="quadrant-label" style="top:8px;right:8px;text-align:right">Vysoký dopad / Vysoká námaha</div>
      <div class="quadrant-label" style="bottom:8px;left:8px">Nízký dopad / Nízká námaha</div>
      <div class="quadrant-label" style="bottom:8px;right:8px;text-align:right">Nízký dopad / Vysoká námaha</div>
      <div class="axis-label axis-x-lo">← Nízká námaha</div>
      <div class="axis-label axis-x-hi">Vysoká námaha →</div>
      <div class="axis-label axis-y-lo">Nízký dopad →</div>
      <div class="axis-label axis-y-hi">← Vysoký dopad</div>
    </div>
  </div>
  <div class="sidebar">
    <h2>📋 Přehled kanálů</h2>
...
<script>
const CATEGORIES = {
  owned:     { label: 'Vlastní kanály',    color: '#7b1fa2' },
  search:    { label: 'Vyhledávání',       color: '#1565c0' },
  paid:      { label: 'Placená reklama',   color: '#e65100' },
  partner:   { label: 'Partnerské',        color: '#2e7d32' },
  offline:   { label: 'Offline & Zážitek', color: '#4a148c' },
  community: { label: 'Komunita & Obsah',  color: '#00838f' },
};

const CHANNELS = [
  { id: 1,  name: 'Email / Newsletter', icon: '📧', cat: 'owned', effort: 20, impact: 88, size: 42, desc: 'Segmentovaný podle chování, fáze vztahu, sezóny. Nejvyšší ROI v e-commerce.', tags: ['Děláte', 'Vysoký ROI', 'Automatizace'] },
  { id: 2,  name: 'SMS', icon: '📱', cat: 'owned', effort: 15, impact: 72, size: 32, desc: 'Jen jednou za kampaň — prostor pro více. Ideální na flash slevové akce.', tags: ['Děláte', 'Krátký text'] },
  { id: 3,  name: 'Push notifikace', icon: '🔔', cat: 'owned', effort: 18, impact: 55, size: 28, desc: 'Web push — nízká saturace v CZ, snadné nastavení přes OneSignal apod.', tags: ['Neděláte', 'Nízká saturace'] },
  { id: 4,  name: 'Věrnostní program / Referral', icon: '🎁', cat: 'owned', effort: 55, impact: 78, size: 38, desc: 'Přiveď kamarádku — virální šíření bez nutnosti platit za dosah.', tags: ['Neděláte', 'Virální potenciál'] },
  { id: 5,  name: 'SEO + Obsahový marketing', icon: '🔍', cat: 'search', effort: 60, impact: 90, size: 46, desc: 'SexTIPy, poradna, návody — dlouhý ocas, organický dosah, budování autority.', tags: ['Děláte', 'Dlouhodobé', 'Autorita'] },
  { id: 6,  name: 'Heureka / Zboží.cz', icon: '🛒', cat: 'search', effort: 22, impact: 58, size: 28, desc: 'Hlavně pro Kondomshop. U erotiky opatrněji — ale funguje pro produkty bez explicity.', tags: ['Děláte (KS)', 'Srovnávače'] },
  { id: 7,  name: 'Google Shopping', icon: '🛍️', cat: 'search', effort: 30, impact: 65, size: 32, desc: 'Vybrané kategorie (pomůcky pro páry, zdraví) — Google je povolí.', tags: ['Neděláte', 'Vybrané kategorie'] },
  { id: 8,  name: 'Rádio', icon: '📻', cat: 'paid', effort: 45, impact: 80, size: 40, desc: 'Nový narativ "plnění přání". Spot schválen Karlem, běží.', tags: ['Děláte', 'Džin narativ', 'Brandový'] },
  { id: 9,  name: 'OOH (billboardy, MHD)', icon: '🪧', cat: 'paid', effort: 50, impact: 68, size: 32, desc: 'Velký potenciál pro odvážnou kreativu. Máte todolist.', tags: ['Plánujete', 'Kreativa'] },
  { id: 10, name: 'Programmatic / Retargeting', icon: '🎯', cat: 'paid', effort: 35, impact: 75, size: 36, desc: 'Sítě přátelské k adult obsahu — Adform, RTB House. Remarketing na návštěvníky.', tags: ['Neděláte', 'Adult-friendly sítě'] },
  { id: 11, name: 'Native advertising', icon: '📰', cat: 'paid', effort: 28, impact: 52, size: 26, desc: 'Seznam Sklik, Taboola — s opatrností. Edukační formáty fungují.', tags: ['Neděláte', 'Sklik'] },
  { id: 12, name: 'Affiliate', icon: '🤝', cat: 'partner', effort: 40, impact: 85, size: 44, desc: 'Právě restrukturalizujete. Nový provizní systém X/Y/Z. Velký potenciál.', tags: ['Děláte', 'Restrukturalizace', 'X=20%'] },
  { id: 13, name: 'Influencer marketing', icon: '⭐', cat: 'partner', effort: 65, impact: 82, size: 40, desc: 'Sex-positive tvůrci, sexuologové, páry. Autentická doporučení fungují lépe než reklama.', tags: ['Neděláte', 'Autenticita'] },
  { id: 14, name: 'Podcasty', icon: '🎙️', cat: 'partner', effort: 55, impact: 70, size: 32, desc: 'Sponzoring nebo vlastní podcast o vztazích, intimitě a zdraví.', tags: ['Neděláte', 'Dlouhý obsah'] },
  { id: 15, name: 'Odborná spolupráce', icon: '👩‍⚕️', cat: 'partner', effort: 60, impact: 78, size: 36, desc: 'Sexuologové, terapeuti, gynekoložky jako ambasadoři — budování důvěry.', tags: ['Neděláte', 'Důvěryhodnost'] },
  { id: 16, name: 'Eventy (EroFest, veletrhy)', icon: '🎪', cat: 'offline', effort: 75, impact: 72, size: 34, desc: 'EroFest — děláte. Wellness veletrhy, párové akce — prostor pro expanzi.', tags: ['Děláte (EroFest)', 'Networking'] },
  { id: 17, name: 'Zážitkové karty', icon: '🎴', cat: 'offline', effort: 50, impact: 65, size: 30, desc: 'Skvělý gifting kanál — dárek pro pár. Máte v todos.', tags: ['Plánujete', 'Gifting'] },
  { id: 18, name: 'B2B (hotely, wellness)', icon: '🏨', cat: 'offline', effort: 70, impact: 60, size: 28, desc: 'Minibary, welcome packety v butikových hotelech. Niche, ale efektní.', tags: ['Neděláte', 'Premium niche'] },
  { id: 19, name: 'YouTube / Video', icon: '▶️', cat: 'community', effort: 72, impact: 80, size: 38, desc: 'SexTIPy, recenze produktů, edukace. Organický dosah bez omezení Mety.', tags: ['Částečně', 'SEO synergies'] },
  { id: 20, name: 'Reddit / Diskuzní fóra', icon: '💬', cat: 'community', effort: 30, impact: 50, size: 24, desc: 'Organická přítomnost v komunitách o sexualitě — nenásilné, autentické.', tags: ['Neděláte', 'Organické'] },
  { id: 21, name: 'TikTok', icon: '🎵', cat: 'community', effort: 78, impact: 70, size: 32, desc: 'Edukační obsah o sexuálním zdraví bez explicity. Mladá generace. Vyžaduje kontinuitu.', tags: ['Neděláte', 'Gen Z', 'Compliance'] },
];
...
function renderBubbles() {
  ...
  CHANNELS.forEach(ch => {
    ...
    bubble.dataset.cat = ch.cat;
    const cat = CATEGORIES[ch.cat];
    const size = ch.size * 2;
    const leftPct = ch.effort;
    const topPct = 100 - ch.impact;
    bubble.style.cssText = \`width: \${size}px; height: \${size}px; background: \${cat.color}; left: \${leftPct}%; top: \${topPct}%; transform: translate(-50%, -50%); font-size: \${ch.size < 30 ? '0.55rem' : '0.62rem'};\`;
    bubble.textContent = ch.icon;
    bubble.title = ch.name;
    bubble.addEventListener('mouseenter', (e) => showTooltip(e, ch));
    bubble.addEventListener('mouseleave', hideTooltip);
    bubble.addEventListener('mousemove', moveTooltip);
    bubble.addEventListener('click', () => selectChannel(ch.id));
...
function applyFilter() {
  document.querySelectorAll('.bubble').forEach(b => {
    const match = activeFilter === 'all' || b.dataset.cat === activeFilter;
    b.classList.toggle('dimmed', !match);
    b.classList.toggle('highlighted', match);
  });
  document.querySelectorAll('.channel-card').forEach(c => {
    c.style.display = (activeFilter === 'all' || c.dataset.cat === activeFilter) ? 'block' : 'none';
  });
}
...
function selectChannel(id) {
  ...
  if (selectedId) {
    document.querySelector(\`.channel-card[data-id="\${selectedId}"]\`)?.scrollIntoView({ behavior: 'smooth', block: 'nearest' });
  }
}

const tooltip = document.getElementById('tooltip');
function showTooltip(e, ch) {
  const cat = CATEGORIES[ch.cat];
  tooltip.innerHTML = \`<h3>\${ch.icon} \${ch.name}</h3><p style="color:\${cat.color};font-size:0.68rem;font-weight:600;margin-bottom:6px">\${cat.label}</p><p>\${ch.desc}</p><div class="meta"><span>Dopad: \${ch.impact}%</span><span>Námaha: \${ch.effort}%</span></div>\`;
...
function renderCards() {
  ...
  const grouped = {};
  CHANNELS.forEach(ch => { if (!grouped[ch.cat]) grouped[ch.cat] = []; grouped[ch.cat].push(ch); });
  Object.entries(grouped).forEach(([cat, channels]) => {
    const info = CATEGORIES[cat];
    channels.sort((a,b) => b.impact - a.impact).forEach(ch => {
      const card = document.createElement('div');
      card.className = 'channel-card';
      card.dataset.id = ch.id;
      card.dataset.cat = ch.cat;
      card.style.background = info.color + '12';
      card.style.color = info.color;
      card.innerHTML = \`<div class="card-header"><span class="card-icon">\${ch.icon}</span><div><div class="card-name">\${ch.name}</div><div class="card-cat">\${info.label}</div></div></div><div class="card-desc">\${ch.desc}</div><div class="card-tags">\${ch.tags.map(t => \`<span class="tag">\${t}</span>\`).join('')}</div>\`;
...
document.querySelectorAll('.filter-btn').forEach(btn => {
  btn.addEventListener('click', () => {
    ...
    document.querySelectorAll('.filter-btn').forEach(b => {
      ...
      const bc2 = bc === 'all' ? '#c2185b' : (CATEGORIES[bc]?.color || '#c2185b');
      if (b === btn) { b.classList.add('active'); b.style.background = bc2; b.style.borderColor = bc2; b.style.color = 'white'; }
      else { b.classList.remove('active'); b.style.background = '#f5f5f5'; b.style.borderColor = 'transparent'; b.style.color = '#555'; }
    });
    applyFilter();
  });
});
...
export default function App() {
  ...
  const TABS = [
    ...
    { key: 'kanaly', label: '📊 Kanály', badge: 0 },
  ]
      ...
      <div className="max-w-3xl mx-auto px-4 py-6">
        ...
        {tab === 'kanaly' && (
          <div style={{ margin: '0 -1rem' }}>
            <iframe
              srcDoc={MARKETING_CHANNELS_HTML}
